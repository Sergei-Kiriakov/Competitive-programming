...
void solve()
{
    ...
    vector<long> cnts(n, x);
    vector<vector<long>> pushed(n);

    long pBooks = m - 1;

    for (int i = n - 1; i >= 0; i--)
    {
        while (cnts[i] > 0 && pBooks >= 0)
        {
            if (books[pBooks] > shelves[i])
            {
                cout << "impossible";
                return;
            }

            pushed[i].push_back(books[pBooks]);
            cnts[i]--;
            pBooks--;
        }
    }
    ...
    for (int i = start; i < n; i++)
    {
        while (cnts[i] < x && l < i)
        {
            if (pushed[i].back() > shelves[l] || cnts[l] <= x - y)
            {
                l++;
            }
            else
