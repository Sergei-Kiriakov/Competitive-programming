...
using namespace SegmentTreeLazy;
using namespace SegmentTreeLazyTraits;
...
void solve()
{
    ...
    auto dfs = [&](long l, long r, auto&& dfs) -> pair<string, long>
    {
        if (l == r)
        {
            return { nm[l], pw[l] };
        ...
        pair<string, long> left = dfs(l, l + (r - l) / 2, dfs);
        pair<string, long> right = dfs(l + (r - l) / 2 + 1, r, dfs);

        if (left.second > right.second)
        {
            if (l == 0 && r == 7)
            {
                cout << left.first << " beats " << right.first;
            }
        ...
        else
        {
            if (l == 0 && r == 7)
            {
                cout << right.first << " beats " << left.first;
            }
