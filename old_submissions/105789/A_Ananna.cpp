...
void solve()
{
    ...
    vector<vector<vector<long>>> eIn(26, vector<vector<long>>(n + 1));
    vector<vector<vector<long>>> eOut(26, vector<vector<long>>(n + 1));

    vector<pair<long, long>> bfs;
    vector<vector<bool>> was(n + 1, vector<bool>(n + 1));

    bfs.reserve(5000 * 5000 + 1);
    ...
    for (int i = 1; i <= m; i++)
    {
        ...
        eIn[s[0] - 'a'][u].push_back(v);
        eOut[s[0] - 'a'][v].push_back(u);
    ...
    {
        pair<long, long> curV = bfs[i];

        for (int j = 0; j < 26; j++)
        {
            for (auto& a : eOut[j][curV.first])
            {
                for (auto& b : eIn[j][curV.second])
                {
                    if (!was[a][b])
                    {
                        was[a][b] = true;
