...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        vector<long> prevDp = dp;

        if (a[i] == 0)
        {
            ans = (ans + prevDp[3] + prevDp[0]) % MOD;

            dp[2] = (dp[2] + prevDp[3]) % MOD;
            dp[2] = (dp[2] + cnt1) % MOD;

            dp[0] = (dp[0] + prevDp[0]) % MOD;
            dp[0] = (dp[0] + cnt0) % MOD;
        ...
        {
            ans = (ans + prevDp[2] + prevDp[1]) % MOD;

            dp[1] = (prevDp[2] + dp[1]) % MOD;
            dp[1] = (dp[1] + cnt0) % MOD;

            dp[3] = (prevDp[1] + dp[3]) % MOD;
            dp[3] = (dp[3] + cnt1) % MOD;
