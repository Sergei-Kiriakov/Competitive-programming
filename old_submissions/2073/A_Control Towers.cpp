...
void solve()
{
    ...
    auto getA = [&](long n) -> long
    {
        return (n * (n - 1) * (n - 2) * (n - 3));
    };

    auto getCnt = [&](vector<string>& area, long n, long m) -> long
    {
        vector<long> cntCol(m);
        vector<long> cntRow(n);

        for (int i = 0; i < n; i++)
        {
            cntRow[i] = count(area[i].begin(), area[i].end(), '.');
            cntRow[i]--;
        }
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            for (int j = 0; j < m; j++)
            {
                ...
                {
                    smCol += cntCol[j];
                    smAll += cntColAll[j] - (2 * cntCol[j] + cntRow[i]);
                }
            }
    ...
    vector<string> areaRev(c, string(r, '0'));
    ...
    cout << getCnt(a, r, c) + getCnt(areaRev, c, r);
}
