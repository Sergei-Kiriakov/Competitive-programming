...
void solve()
{
    ...
    vector<vector<long>> dp(n + 1, vector<long>(n + 1, 1e18));

    for (int len = 1; len <= n; len++)
    {
        for (int last = n; last >= len; last--)
        {
            if (len == 1)
            {
                dp[last][last] = 0;
            ...
            {
                for (int lenLeft = 1; lenLeft <= len - 1; lenLeft++)
                {
                    long l = last - len + 1;
                    long mid = last - len + 1 + lenLeft - 1;

                    dp[l][last] = min(dp[l][last], dp[l][mid] + abs(pos[l] - pos[mid + 1]) + dp[mid + 1][last]);
                }
            }
        }
    }
