...
void solve()
{
    ...
    vector<long> dst(n + 1, 1e18);
    dst[1] = 0;
    
    set<pair<long, long>> dj;

    dj.insert({0, 1});

    while (dj.size())
    {
        pair<long, long> cur = *dj.begin();
        dj.erase(cur);

        if (cur.first == dst[cur.second])
        {
            for (auto& next : g[cur.second])
            {
                if (dst[next.second] > dst[cur.second] + next.first)
                {
                    dst[next.second] = dst[cur.second] + next.first;
                    dj.insert({dst[next.second], next.second});
                }
            }
        }
    }
    ...
    for (int i = 2; i <= n; i++)
    {
        for (auto& next : g[i])
        {
            if (dst[i] - next.first == dst[next.second])
            {
                tree[next.second].push_back(i);
                break;
            }
        }
    }
    ...
    auto dfs = [&](long curV, auto&& dfs) -> long
    {
        long sm = 0;

        for (auto& next : tree[curV])
        {
            long smNext = dfs(next, dfs);

            ans += (dst[next] - dst[curV]) * smNext;
