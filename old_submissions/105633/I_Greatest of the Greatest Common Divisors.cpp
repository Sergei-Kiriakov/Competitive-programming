...
void solve()
{
    ...
    vector<vector<pair<long, long>>> rs(n + 1);
    ...
    vector<vector<long>> delPos(1e5 + 1);
    vector<multiset<long>> vals(n + 1);

    vector<vector<pair<long, long>>> toErase(n + 1);
    ...
    for (int i = n; i >= 1; i--)
    {
        for (int j = 1; j * j <= a[i]; j++)
        {
            if (a[i] % j == 0)
            {
                if (delPos[j].size())
                {
                    long posJ = delPos[j].back();
                    toErase[i].push_back({ j, posJ });

                    vals[posJ].insert(j);
                }

                if (delPos[a[i] / j].size())
                {
                    long posDJ = delPos[a[i] / j].back();
                    toErase[i].push_back({ a[i] / j, posDJ});

                    vals[posDJ].insert(a[i] / j);
                }

                delPos[j].push_back(i);
                delPos[a[i] / j].push_back(i);
            }
        }
    }
    ...
    SegmentTree::SegmentTree<long, SegmentTree::TraitsMaxSet<long>> segMax;
    segMax.build(arr);
    ...
    for (int l = 1; l <= n; l++)
    {
        for (auto& query : rs[l])
        {
            ans[query.second] = segMax.get(l, query.first);
        }

        for (auto& er : toErase[l])
        {
            vals[er.second].erase(vals[er.second].lower_bound(er.first));

            segMax.update(er.second, *vals[er.second].rbegin());
        }
    }
