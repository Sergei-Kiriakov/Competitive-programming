...
void solve()
{
    ...
    vector<long> posMax(k + 1, -1);
    vector<long> cnts(k + 1);
    ...
    set<pair<long, long>> freeCol;
    set<pair<long, long>> nminPos;

    for (int i = 1; i <= k; i++)
    {
        if (m - posMax[i] >= p || posMax[i] == -1)
        {
            freeCol.insert({ cnts[i], i});
        }
        else nminPos.insert({posMax[i], i});
    }

    while (a.size() != n)
    {
        while (nminPos.size() && (long)a.size() - nminPos.begin()->first >= p)
        {
            pair<long, long> cur = *nminPos.begin();
            nminPos.erase(cur);

            freeCol.insert({ cnts[cur.second], cur.second });
        }

        if (freeCol.size())
        {
            pair<long, long> need = *freeCol.begin();
            freeCol.erase(need);

            long col = need.second;

            cnts[col]++;
            nminPos.insert({a.size(), col});

            a.push_back(need.second);
        }
        else
        {
            cout << "impossible";
    ...
    posMax = vector<long>(k + 1, -1);

    for (int i = 0; i < n; i++)
    {
        if (posMax[a[i]] != -1 && i - posMax[a[i]] < p)
        {
            cout << "impossible";
            return;
        }
