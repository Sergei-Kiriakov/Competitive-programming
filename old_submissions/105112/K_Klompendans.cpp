...
void solve()
{
    ...
    vector<vector<vector<bool>>> was(n + 1, vector<vector<bool>>(n + 1, vector<bool>(2)));

    queue<pair<long, pair<long, bool>>> bfs;
    ...
    vector<pair<long, long>> choicesA = { {a, b}, {b, a}, {-a, b}, {b, -a}
        , {a, -b}, {-b, a}, {-a, -b}, {-b, -a} },
        choicesB = { {c, d}, {d, c}, {-c, d}, {d, -c}
        , {c, -d}, {-d, c}, {-c, -d}, {-d, -c} };

    auto canMove = [&](long a, long b) -> bool
    {
    ...
    while (bfs.size())
    { 
        pair<long, pair<long, bool>> state = bfs.front();
        bfs.pop();

        long i = state.first, j = state.second.first;
        bool useA = state.second.second;

        if (useA)
        {
            for (auto& dir : choicesA)
            {
                pair<long, pair<long, bool>> nextState;
                nextState.first = state.first + dir.first;
                nextState.second.first = state.second.first + dir.second;
                nextState.second.second = false;

                if (canMove(nextState.first, nextState.second.first) && !was[nextState.first][nextState.second.first][nextState.second.second])
                {
                    bfs.push(nextState);
                    was[nextState.first][nextState.second.first][nextState.second.second] = true;
                }
            }
        }
        else
        {
            for (auto& dir : choicesB)
            {
                pair<long, pair<long, bool>> nextState;
                nextState.first = state.first + dir.first;
                nextState.second.first = state.second.first + dir.second;
                nextState.second.second = true;
